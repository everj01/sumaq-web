...
import { type DatePickerProps, type UploadProps, type  RadioChangeEvent } from 'antd';
...
const props: UploadProps = {
  ...
  onChange(info) {
    ...
    if (!isJpgOrPng) {
      message.open({
        type: 'warning',
        content: 'Solo se permiten imágenes JPG/PNG',
        duration: 2
      });
      return;
    }
    ...
    if (status === 'done') {
      message.open({
        type: 'success',
        content: 'Foto subida correctamente',
        duration: 2
      });
      console.log(`${info.file.name} Foto mubida correctamente.`);
    } else if (status === 'error') {
       message.open({
        type: 'error',
        content: 'No se pudo subir',
        duration: 2
      });
      console.log(`${info.file.name} La foto no se pudo subir.`);
    }
  },
  onDrop(e) {
    console.log('Archivos eliminados', e.dataTransfer.files);
  },
};
            ...
            >
              <div className="grow max-h-[500px] grid grid-cols-1 lg:grid-cols-2 gap-x-3 gap-y-0 w-full  px-5 overflow-y-auto">
                ...
                rules={[
                  ...
                  <DatePicker onChange={onChangeBirthday} className="w-full" placeholder="Seleccione la fecha"/>
                </Form.Item>
                ...
                <Form.Item 
                name="email"
                label="Correo Electronico" rules={[
